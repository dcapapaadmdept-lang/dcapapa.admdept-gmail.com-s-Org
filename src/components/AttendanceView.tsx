...
export default function AttendanceView({
  ...
  const handleSaveRegister = async () => {
    ...
    console.log('[DATABASE OP] Attempting UPSERT on "department_attendance"', {
      operation: 'UPSERT',
    ...
    try {
      ...
      console.error('[DATABASE FAULT] Failed to upsert into "department_attendance"', {
        operation: 'UPSERT',
        payload: records,
        error: err
      });
      alert(`Database Save Failed on 'department_attendance': ${err.message || JSON.stringify(err)}`);
    }
  };
  ...
  const exportAttendancePDF = () => {
    ...
    doc.setFont('Helvetica', 'bold');
    doc.setFontSize(13);
    doc.text('MEMBER ATTENDANCE - GENERAL SUMMARY REGISTER', 14, 32);
    
    doc.setFontSize(9.5);
    doc.setFont('Helvetica', 'normal');
    const roleLimit = isDeptHead ? `Dept: ${departments.find(d=>d.id===myDeptId)?.department_name}` : 'Church-Wide';
    doc.text(`Generated By: ${activeProfile.full_name} (${activeProfile.role}) | Scope: ${roleLimit}`, 14, 39);
    doc.text(`Date of Download: ${new Date().toLocaleDateString()}`, 14, 44);
